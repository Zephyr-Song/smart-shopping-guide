...
export const KB_ITEMS: KbItem[] = [...STORE_ITEMS, ...FACILITIES, ...SERVICES, ...FAQ, ...POLICIES]
 ...
 * 纯关键词/二元组打分，足够覆盖"店铺/设施/服务/政策"类问答；
...
export function retrieve(query: string, k = 6): string {
  ...
  // 至少返回一些内容，避免完全无上下文
  const picked = top.every(s => s.score === 0) ? scored.slice(0, k) : top
  return picked.map(s => `[${s.item.category}] ${s.item.title}\n${s.item.content}`).join('\n\n---\n\n')
}
